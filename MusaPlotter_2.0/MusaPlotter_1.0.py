...
#from PyQt4.QtGui import QApplication, QDialog, QTextCursor, QActionGroup
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def setTime(self,serialComClient):
        ...
        utctime = time.time()
        sec = int(time.strftime("%S",time.localtime(utctime)))
        if sec > 54: #if close to minute rollover
            time.sleep(6) #wait a few seconds or else the set time will be 1 minute behind
            utctime = time.time() #and take another reading
        utctime = utctime+60 #add 1 minute
        timestamp = str(time.strftime("%y%m%d%H%M%S%w",time.localtime(utctime)))
        yy = timestamp[0:2]
        mo = timestamp[2:4]
        dd = timestamp[4:6]
        hh = timestamp[6:8]
        ...
        #send message here with **TIME**:MN:HH:DW:DM:MT:YR
    ...
    def addChanBoxes(self, nameList): #load the channels into the UI
        ...
        for i in range(len(nameList)):
            ...
            #connect signals:
            #QtCore.QObject.connect(self.scaleBoxes[i], QtCore.SIGNAL('valueChanged(double)'), lambda : self.updatePlotLineScale2())
            self.scaleBoxes[i].valueChanged.connect(lambda: self.updatePlotLineScale2())
            #QtCore.QObject.connect(self.showSeriesBoxes[i], QtCore.SIGNAL('clicked()'), lambda : self.updatePlotLineScale2())
            self.showSeriesBoxes[i].clicked.connect(lambda: self.updatePlotLineScale2())
            #buttons with names for average capture:
            #QtCore.QObject.connect(self.buttons[i], QtCore.SIGNAL('clicked()'), lambda boxName = self.readingBoxes, num = i: caputre(boxName, num, self.serialComClient))
            self.buttons[i].clicked.connect(lambda dummy, boxName = self.readingBoxes, num = i: caputre(boxName, num, self.serialComClient))
            #zero boxes, spanboxes (method to uncheck)
            #QtCore.QObject.connect(self.zeroBoxes[i], QtCore.SIGNAL('clicked()'), lambda boxName = self.zeroBoxes, boxNameOther = self.spanBoxes, num = i: self.toggleCheck(boxName,boxNameOther, num))
            ...
            #QtCore.QObject.connect(self.spanBoxes[i], QtCore.SIGNAL('clicked()'), lambda boxName = self.spanBoxes, boxNameOther = self.zeroBoxes, num = i: self.toggleCheck(boxName,boxNameOther, num))
...
#                                    self.valChans.append(splitValc[0] )
                ...
                if self.task[0] == "**TIME**": #cal      5     Change? (y/n)> y      minutes>   hour>    Day of week (1-7)>     Day of month>     Month>      Year (2 digit) >   Set? (y/n)>    
                    ...
                    if self.task[-1] == "sayYestoChange" and "Change? (y/n)>" in self.line:
                        ...
                        self.ser.write("y\n".encode())
                        self.task[-1] = "putMiniutes"
                    
                    if self.task[-1] == "putMiniutes" and "minutes>" in self.line:
                        print("<bkd> minutes>")
                        self.queueRecv.put("<bkd> sending mins rtn ")
                        self.ser.write(self.task[1].encode())##timeset put self.task[x] once sent proper format setup
                        self.ser.write("\n".encode())
                        self.task[-1] = "putHour"
                        
                    if self.task[-1] == "putHour" and "hour>" in self.line:
                        print("<bkd> hour>")
                        self.queueRecv.put("<bkd> sending hrs rtn ")
                        self.ser.write(self.task[2].encode())##timeset put self.task[x] once sent proper format setup
                        self.ser.write("\n".encode())
                        self.task[-1] = "putDayW"
                        
                    if self.task[-1] == "putDayW" and "Day of week (1-7)>" in self.line:
                        print("<bkd> dayW>")
                        self.queueRecv.put("<bkd> sending dayW rtn ")
                        self.ser.write(self.task[3].encode())##timeset put self.task[x] once sent proper format setup
                        self.ser.write("\n".encode())
                        self.task[-1] = "putDayM"
                        
                    if self.task[-1] == "putDayM" and "Day of month>" in self.line:
                        print("<bkd> dayM>")
                        self.queueRecv.put("<bkd> sending dayM rtn ")
                        self.ser.write(self.task[4].encode())##timeset put self.task[x] once sent proper format setup
                        self.ser.write("\n".encode())
                        self.task[-1] = "putMonth"
                    
                    if self.task[-1] == "putMonth" and "Month>" in self.line:
                        print("<bkd> month>")
                        self.queueRecv.put("<bkd> sending mnth rtn ")
                        self.ser.write(self.task[5].encode())##timeset put self.task[x] once sent proper format setup
                        self.ser.write("\n".encode())
                        self.task[-1] = "putYear" 
                        
                    if self.task[-1] == "putYear" and "Year (2 digit) >" in self.line:
                        print("<bkd> year>")
                        self.queueRecv.put("<bkd> sending year rtn ")
                        self.ser.write(self.task[6].encode())##timeset put self.task[x] once sent proper format setup
                        self.ser.write("\n".encode())
                        self.task[-1] = "putsetY" 
                    
                    if self.task[-1] == "putsetY" and "Set? (y/n)>" in self.line:
                        print("<bkd> set Y>")
                        self.queueRecv.put("<bkd> waiting for the next minute")
                        while(1):
                            if str(time.strftime("%S")) == '58': #if the minute is about to tick
                                self.queueRecv.put("<bkd> sending y ")
                                self.ser.write("y\n".encode())
