...
def field_SubtractBkg(inputpath, energyinputpath, outputpath, aveoutputpath, timespath, logpath):
    ver = '0.3'
    ...
    potentialBkgNames = ['CO', 'CO2', 'PM', 'VOC',
                         'CH4']  # define potential channel names that will get background subtraction
   ...
   '''
    ...
    if os.path.isfile(timespath):
        ...
        try:
            dateobject = data['dateobjects'][0] + timedelta(hours=0,
                                                            minutes=4)  # time series data start time plus 4 minutes
        ...
        eunits[name] = 'hh:mm:ss'
        try:
            dateobject = dt.strptime(eval['start_time_hp'], '%H:%M:%S') - timedelta(hours=0,
                                                                                    minutes=2)  # hp start time minus 2 minutes
            eval[name] = dateobject.strftime('%H:%M:%S')
        except:
            eval[name] = ''
        eunc[name] = ''
    ...
    [phasedatenums, phasedata, phasemean] = definePhaseData(names, data, phases,
                                                            phaseindices)  # define phase data series for each channel

    bkgvalue = calcBkgValue(bkgnames,
                            phasemean)  # calculate the background value for each channel that will get background subtraction

    data_new = bkgSubtraction(names, data, bkgnames, bkgvalue)  # subtract the background

    [phasedatenums, phasedata_new, phasemean_new] = definePhaseData(names, data_new, phases,
                                                                    phaseindices)  # define phase data series after background subtraction
...
if __name__ == "__main__":
    field_SubtractBkg(inputpath, energyinputpath, outputpath, aveoutputpath, timespath, logpath)
