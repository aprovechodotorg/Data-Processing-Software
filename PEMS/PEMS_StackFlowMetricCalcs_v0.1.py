...
def PEMS_StackFlowMetricCalcs(inputpath,energypath,carbalpath,metricpath,logpath):
    ...
    for em in emissions:
        ...
        #integrate the emissions rate series
        dt = 1  #time step, add code to read dt from time series
        ...
        if len(unclist) == 0:   #if no uncertainty
            ...
            uncle = sum(unclist)/len(unclist)   #metric unc assuming perfect correlation between time series values
