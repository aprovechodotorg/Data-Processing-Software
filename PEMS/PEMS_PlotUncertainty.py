...
def PEMS_PlotUncertainty(names, units, data, fnames, exnames, snames, nnames, tnames, sennames, plotpath, savefig):
        ...
        for name in plotnames:
            ...
            try:
                for m, val in enumerate(data[name]):
                    maximum = val + data[ucname][m]
                    maxdata.append(maximum)
                    minimum = val - data[ucname][m]
                    mindata.append(minimum)

                #lighten assigned color
                try:
                    c = mc.cnames[colors[name]] #check that color is a color name
                except:
                    c = colors[name] #rgb value
                c = colorsys.rgb_to_hls(*mc.to_rgb(c))
                c = colorsys.hls_to_rgb(c[0], 1 - 0.7 * (1 - c[1]), c[2])

                ax.plot(data['datenumbers'], maxdata, color=c, linewidth=lw/2, linestyle='dashed')
                ax.plot(data['datenumbers'], mindata, color=c, linewidth=lw/2, linestyle='dashed')
            except:
                line = 'No uncertainty data found for ' + name
                print(line)
            ...
            if typename == name:
                ...
                for x, date in enumerate(data[type + 'datenumbers']):  # cut data to phase time
                    if start <= date <= end:
                        ...
                        try:
                            maximum = data[name][x] + data[ucname][x]
                            maxdata.append(maximum)
                            minimum = data[name][x] - data[ucname][x]
                            mindata.append(minimum)
                        except:
                            pass
                ...
                try:
                    ax.plot(datenumbers, maxdata, linewidth=lw/2, color=c, linestyle='dashed')
                    ax.plot(datenumbers, mindata, linewidth=lw/2, color=c, linestyle='dashed')
                except:
                    line = 'No uncertainty data found for ' + name
                    print(line)
...
if __name__ == "__main__":
    PEMS_PlotUncertainty(names, units, data, fnames, exnames, snames, nnames, tnames, sennames, plotpath, savefig)
