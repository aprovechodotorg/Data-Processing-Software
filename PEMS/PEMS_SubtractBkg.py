...
def PEMS_SubtractBkg(inputpath, energyinputpath, ucpath, outputpath, aveoutputpath, timespath, bkgmethodspath, logpath,
    ...
    potentialBkgNames = ['CO', 'CO2', 'PM', 'COhi', 'CO2hi', 'NO', 'NO2', 'VOC',
                         'HC']  # define potential channel names that will get background subtraction
    ...
    if os.path.isfile(timespath):
        ...
        try:
            if timeformatstring == 'hh:mm:ss':
                dateobject = dt.strptime(starttime, '%H:%M:%S') - timedelta(hours=0,
                                                                            minutes=2)  # start time minus 2 minutes
                eval[name] = dateobject.strftime('%H:%M:%S')
            else:
                dateobject = dt.strptime(starttime, '%Y%m%d %H:%M:%S') - timedelta(hours=0,
                                                                                   minutes=2)  # start time minus 2 minutes
                eval[name] = dateobject.strftime('%Y%m%d %H:%M:%S')
        except:
            eval[name] = ''
        ...
        eunits[name] = timeformatstring
        try:
            dateobject = data['dateobjects'][-1] - timedelta(hours=0,
                                                             minutes=12)  # time series data end time minus 12 minutes
            if timeformatstring == 'hh:mm:ss':
                eval[name] = dateobject.strftime('%H:%M:%S')
            else:
                eval[name] = dateobject.strftime('%Y%m%d %H:%M:%S')
        ...
        try:
            dateobject = data['dateobjects'][-1] - timedelta(hours=0,
                                                             minutes=2)  # time series data end time minus 2 minutes
            if timeformatstring == 'hh:mm:ss':
                eval[name] = dateobject.strftime('%H:%M:%S')
            else:
                eval[name] = dateobject.strftime('%Y%m%d %H:%M:%S')
        ...
        if Name in Bkgnames:  # that will get background subtraction
            ...
            if Methods[Name] == 'pre':
                ...
                try:
                    ...
                    Bkgvalue[Name] = np.mean([Phasemean[Name + '_prebkg'], Phasemean[Name + '_postbkg']]) - Offsets[
                        Name]
                for n in Data[Name]:
                    Data_bkgseries[Name].append(Bkgvalue[Name])
