...
import matplotlib
import easygui
from PEMS_PlotTimeSeries2 import PEMS_PlotTimeSeries
import sys
import os
...
try:
    for n,val in enumerate(data['time']):
        ...
        try:
            for n,val in enumerate(data['time']):
                dateobject=dt.strptime(val, '%H:%M:%S')
                data[name].append(dateobject)  
        except:
            print('ERROR: Could not read time format')
...
PEMS_PlotTimeSeries(names,units,data,title)    #send data to plot function