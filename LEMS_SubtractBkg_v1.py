...
#maybe create blank phase times input file if one does not exist
#move plots to plot library
#add additional figure for CH4 and VOC
...
def LEMS_SubtractBkg(inputpath,outpath,timespath,logpath):
    ...
    [phasedatenums,phasedata,phasemean] = definePhaseData(names,data,phases,phaseindices,datenums)   #define phase data series for each channel
    ...
    [phasedatenums,phasedata_new,phasemean_new] = definePhaseData(names,data_new,phases,phaseindices,datenums)   #define phase data series after background subtraction
    ...
    for i, ax in enumerate(f1.axes):
        name=plotnames[i]
        ax.plot(datenums,data[name],color='silver',linewidth=lw, label='raw_data')   #original data series
        ax.plot(datenums,data_new[name],color='k',linewidth=lw,label='bkg_subtracted')   #bkg subtracted data series
        for phase in phases:
            ...
            ax.plot([phasedatenums[phase][0],phasedatenums[phase][-1]],[phasedata[phasename][0],phasedata[phasename][-1]],color=colors[phase],linestyle='none',marker='|',markersize=msize)
            ax.plot(phasedatenums[phase],phasedata_new[phasename],color=colors[phase],linewidth=plw)    #original          
...
def findIndices(Timenames,Timeobject,Datenums):
    datenumbers={}
    Indices={}
    for Name in Timenames:
...
def definePhaseData(Names,Data,Phases,Indices,Datenums):
    Phasedatenums={}
    Phasedata={}
    Phasemean={}
    for Phase in Phases: #for each test phase
        ...
        Phasedatenums[Phase]=Datenums[startindex:endindex+1]    
