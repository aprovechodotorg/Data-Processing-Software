...
def LEMS_3002(Inputpath, outputpath, headerpath, logger):
    ...
    names_new = ['time', 'seconds', 'CO', 'CO2', 'PM', 'Flow', 'FLUEtemp', 'H2Otemp', 'RH', 'COtemp', 'TC1', 'dP2',
                 'O2_1', 'O2_2', 'O2_3', 'O2_4']  # New list for names
    ...
    for name in names_new:  # Different variables have different calculations for unit conversion
        ...
        if name == 'CO':
            ...
            for val in data['flue T']:   # By multiplier
                try:
            ...
            for val in data['O2 ch1']:   # No change
                values.append(val)
        elif name == 'O2_2':
            for val in data['O2 ch2']:  # No change
                values.append(val)
        elif name == 'O2_3':
            for val in data['O2 ch3']:  # No change
                values.append(val)
        elif name == 'O2_4':
            for val in data['O2 ch4']:  # No change
                values.append(val)
        elif name == 'seconds':
...
if __name__ == "__main__":
    LEMS_3002(inputpath, outputpath, headerpath, logger)
