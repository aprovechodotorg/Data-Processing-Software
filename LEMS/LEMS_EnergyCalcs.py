...
def LEMS_EnergyCalcs(inputpath, outputpath, logger):
    ...
    for phase in phases:
        ...
        metrics = []  # initialize list of phase-specific metrics (that will get renamed with phase identifier and put
        # in 'names')
        ...
        try:  # Current multi fuel sheet - use EHV without charcoal production factored in
            ...
            log = 'TE without char equation: useful energy delivered / (fuel mass * fuel effective heating value ' \
                  'without char) * 100'
            print(log)
            logger.info(log)
            logs.append(log)
        ...
        try:  # Typical equation. Current spreadsheet if charcoal is entered in char space and not as a multi fuel
            ...
            log = 'TE with char equation: useful energy delivered / (fuel mass * fuel effective heating value - ' \
            ...
            try:  # try multi fuel calculations where EHV includes char production
                ...
                try:  # try the older spreadsheet where the effective heating value was entered for the fuel
                    ...
                    log = 'TE with char equation: useful energy delivered / (fuel mass * fuel heating value ' \
                    ...
                    try:  # try calculating without the char values (they may not be entered or may be 0)
                        pval[name] = pval['useful_energy_delivered'] / \
                                     (pval['fuel_mass'] * uval['fuel_heating_value']) * 100
        ...
        metrics.append(name)
        try:
            pval[name] = (pval['fuel_mass'] * pval['fuel_EHV'] - pval['char_mass'] * uval['char_lower_heating_value']) \
                         / (pval['phase_time'] * 60)
        except KeyError:
            try:
