...
# quality_frame.grid(row=3, column=0, padx=0, pady=0)
    ...
    def on_cut_plot(self):
        ...
        self.cscalepath = os.path.join(self.found_folder_path,
                                         f'{os.path.basename(self.found_folder_path)}_FormattedCombinedScaleData.csv')
        self.mscalepath = os.path.join(self.found_folder_path,
                                       f'{os.path.basename(self.found_folder_path)}_FormattedMTScaleData.csv')
                ...
                try:
                    ...
                    names, units, data, fnames, fcnames, exnames, snames, isnames, anames, cnames, mnames, nnames,\
                        ...
                        PEMS_Plotter(self.input_path, self.fuel_path, self.fuelmetric_path, self.exact_path,
                                     self.scale_path, self.intscale_path, self.ascalepath, self.cscalepath,
                                     self.mscalepath, self.nano_path, self.teom_path, self.senserion_path,
                    ...
                    try:
                        [x_variable, y_variable, figpath] = \
                            LEMS_customscatterplot(self.input_path, self.fuel_path, self.exact_path, self.scale_path,
                                                   self.intscale_path, self.ascalepath, self.cscalepath,
                                                   self.mscalepath, self.nano_path, self.teom_path,
    ...
    def on_bc(self):
        try:
            self.inputpath = os.path.join(self.found_folder_path,
                                          f"{os.path.basename(self.found_folder_path)}_BCInputs.csv")
            self.outputpath = os.path.join(self.found_folder_path,
                                           f"{os.path.basename(self.found_folder_path)}_BCOutputs.csv")
            self.grav_inputpath = os.path.join(self.found_folder_path,
                                               f"{os.path.basename(self.found_folder_path)}_GravInputs.csv")
            self.grav_outputpath = os.path.join(self.found_folder_path,
                                                f"{os.path.basename(self.found_folder_path)}_GravOutputs.csv")
    ...
    def save(self):
        ...
        try:
            names, units, data, fnames, fcnames, exnames, snames, isnames, anames, cnames, mnames, nnames, tnames,\
                ...
                PEMS_Plotter(self.input_path, self.fuel_path, self.fuelmetric_path, self.exact_path,
                             self.scale_path, self.intscale_path, self.ascalepath, self.cscalepath, self.mscalepath,
        ...
        for key, value in sorted_data.items():
            if key.startswith('variable'):
                ...
                if equation_calculations[0]:
                    message = equation_calculations
                    formula = equation_calculations

                    info_icon = tk.Label(self.text_widget, text="ⓘ", fg="blue", cursor="hand2", font=("Helvetica",
                    info_icon.bind("<Enter>", lambda e, m = message, f = formula: self.show_info_popup_right(m, e.widget, formula = f))
...
class BC_Calcs(tk.Frame):
    def __init__(self, root, logs, data, units, pic_list):
        tk.Frame.__init__(self, root)
        # Exit button
        exit_button = tk.Button(self, text="EXIT", command=root.quit, bg="red", fg="white")
        exit_button.grid(row=0, column=4, padx=(410, 5), pady=5, sticky="e")
    ...
    def __init__(self, root, data, units, logs, num_columns, num_rows, folder_path):
        ...
        for key, value in sorted_data.items():
            ...
            if key.startswith('firepower_w_char_mp'):
                print(self.warning_frame.cget("state"))
                if val:
                    result = False
                    try:
                    ...
                    self.warning_frame.insert(tk.END, 'ISO FAULT:\n')
                    warning_message_1 = f"  {key} is not between high power and low power. ISO tests REQUIRE medium power firepower to be between high and low power.\n"
                    warning_message = warning_message_1
