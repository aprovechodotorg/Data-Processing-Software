...
def LEMS_Int_Scale(inputpath, outputpath, logpath):
    ...
    for n, row in enumerate(stuff[:100]): #iterate through first 101 rows to look for start
            if 'Timestamp' in row[0]:
                timerow = n
            if ('ST' in row[0] or'US' in row[0]) and x == 0: #only do this the first time ST or US is found
                datarow = n
                x = 1
    ...
    tempdata = [x[1] for x in stuff[datarow:]] #assign first column as temporary data
    data['weight'] = []

    for row in tempdata:
        try:
            weight, unit = row.split(" ") #split at the space: data has format + then value then unit
