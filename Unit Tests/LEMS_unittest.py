...
# python -m unittest Unit Tests.LEMS_unittest
# or
# python -m unittest <Unit Tests folder>.<unittest file name>
import unittest
import csv
...
class LEMS_unittest(unittest.TestCase):
    ...
    def test_energycalcs_4003_ISO(self):
        base_path = Path("./Unit Tests/alcohol_test1")
    ...
    #############################################################################
    #SB4003 TESTS - IDC - Old datasheet
    def test_energycalcs_4003_IDC(self):
        base_path = Path("./Unit Tests/5.31")
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Energy Inputs - SB4003 IDC 5.31")

        #for calculated_line, known_line in zip(calculated_file, known_file):
            #self.assertEqual(calculated_line, known_line)

        for n, row in enumerate(known):
            message = ('Row ' + str(n +1) + ' of known energy input file and calculated energy input file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
        ...
        if (known_linecount - calculated_linecount) == 1: #There's one known added line in know energy output for SB firmware
            known_linecount = known_linecount - 1
            known.pop(-1)
        self.assertEqual(calculated_linecount, known_linecount, f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Energy Outputs - SB4003 IDCc5.31")

        #for calculated_line, known_line in zip(calculated_file, known_file):
            #self.assertEqual(calculated_line, known_line)

        for n, row in enumerate(known):
            message = ('Row ' + str(n + 1) + ' of known energy output file and calculated energy output file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_AdjustCalibrations_4003_IDC(self):
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Recalibrated Raw Data - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known recalibrated file and calculated recalibrated file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Header - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known header file and calculated header file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_shifted_response_4003_ISO(self):
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Shifted Raw Data - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known shifted file and calculated shifted file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for TimeShifts - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known timeshifts file and calculated timeshifts file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_bkg_subtract_4003_IDC(self):
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for UC Inputs - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known UC inputs file and calculated UC inputs file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for TimeSeries - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known timeseries file and calculated timeseries file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Averages - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known averages file and calculated averages file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_grav_calcs_4003_IDC(self):
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Grav Outputs - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known grav outputs file and calculated grav outputs file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_emission_calcs_4003_IDC(self):
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Emission Outputs - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known emission outputs file and calculated emission outputs file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for All Outputs - SB4003 IDC 5.31")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known all outputs file and calculated all outputs file are not equal - SB4003 IDC 5.31.')
            self.assertEqual(row, calculated[n], message)
    ...
    #############################################################################
    #SB4003 TESTS - IDC - new/multi fuel datasheet
    def test_energycalcs_4003_IDC_new(self):
        base_path = Path("./Unit Tests/10.19")

        known_data_entry_path = base_path / "10.19_TE_dataEntrySheet.xlsx"

        calculated_energy_inputs_path = base_path / "10.19_EnergyInputs_test.csv"
        known_energy_inputs_path = base_path / "10.19_EnergyInputs.csv"

        calculated_energy_outputs_path = base_path / "10.19_EnergyOutputs_test.csv"
        known_energy_outputs_path = base_path / "10.19_EnergyOutputs.csv"

        log_path = base_path / "10.19_log.txt"
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Energy Inputs - SB4003 IDC 10.19")

        #for calculated_line, known_line in zip(calculated_file, known_file):
            #self.assertEqual(calculated_line, known_line)

        for n, row in enumerate(known):
            message = ('Row ' + str(n +1) + ' of known energy input file and calculated energy input file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
        ...
        if (known_linecount - calculated_linecount) == 1: #There's one known added line in know energy output for SB firmware
            known_linecount = known_linecount - 1
            known.pop(-1)
        self.assertEqual(calculated_linecount, known_linecount, f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Energy Outputs - SB4003 IDC 10.19")

        #for calculated_line, known_line in zip(calculated_file, known_file):
            #self.assertEqual(calculated_line, known_line)

        for n, row in enumerate(known):
            message = ('Row ' + str(n + 1) + ' of known energy output file and calculated energy output file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_AdjustCalibrations_4003_IDC_new(self):
        base_path = Path("./Unit Tests/10.19")

        known_raw_data_path = base_path / "10.19_RawData.csv"
        known_energy_outputs_path = base_path / "10.19_EnergyOutputs.csv"

        calculated_recalibrated_path = base_path / "10.19_RawData_Recalibrated_test.csv"
        known_recalibrated_path = base_path / "10.19_RawData_Recalibrated.csv"

        calculated_header_path = base_path / "10.19_Header_test.csv"
        known_header_path = base_path / "10.19_Header.csv"
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Recalibrated Raw Data - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known recalibrated file and calculated recalibrated file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Header - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known header file and calculated header file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_shifted_response_4003_ISO_new(self):
        base_path = Path("./Unit Tests/10.19")

        known_raw_data_path = base_path / "10.19_RawData_Recalibrated.csv"

        calculated_shifted_path = base_path / "10.19_RawData_Shifted_test.csv"
        known_shifted_path = base_path / "10.19_RawData_Shifted.csv"

        calculated_timeshifts_path = base_path / "10.19_TimeShifts_test.csv"
        known_timeshifts_path = base_path / "10.19_TimeShifts.csv"
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Shifted Raw Data - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known shifted file and calculated shifted file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for TimeShifts - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known timeshifts file and calculated timeshifts file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_bkg_subtract_4003_IDC_new(self):
        base_path = Path("./Unit Tests/10.19")

        known_raw_data_path = base_path / "10.19_RawData_Shifted.csv"
        known_energy_input_path = base_path / "10.19_EnergyInputs.csv"
        known_phasetime_path = base_path / "10.19_PhaseTimes.csv"
        known_bkg_methods_path = base_path / "10.19_BkgMethods.csv"
        known_fig1_path = base_path / "10.19_subtractbkg1.png"
        known_fig2_path = base_path / "10.19_subtractbkg2.png"

        known_UC_path = base_path / "10.19_UCInputs.csv"
        calculated_UC_path = base_path / "10.19_UCInputs_test.csv"

        calculated_timeseries_path = base_path / "10.19_test_TimeSeries.csv"
        known_timeseries_path = base_path / "10.19_TimeSeries.csv"

        calculated_averages_path = base_path / "10.19_Averages_test.csv"
        known_averages_path = base_path / "10.19_Averages.csv"
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for UC Inputs - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known UC inputs file and calculated UC inputs file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for TimeSeries - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known timeseries file and calculated timeseries file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Averages - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known averages file and calculated averages file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_grav_calcs_4003_IDC_new(self):
        base_path = Path("./Unit Tests/10.19")

        known_grav_input_path = base_path / "10.19_GravInputs.csv"
        known_averages_path = base_path / "10.19_Averages.csv"
        known_phasetime_path = base_path / "10.19_PhaseTimes.csv"
        known_energy_output_path = base_path / "10.19_EnergyOutputs.csv"

        known_grav_output_path = base_path / "10.19_GravOutputs.csv"
        calculated_grav_output_path = base_path / "10.19_GravOutputs_test.csv"
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Grav Outputs - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known grav outputs file and calculated grav outputs file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_emission_calcs_4003_IDC_new(self):
        base_path = Path("./Unit Tests/10.19")
        ...
        known_emission_output_path = base_path / "10.19_EmissionOutputs.csv"
        calculated_emission_output_path = base_path / "10.19_EmissionOutputs_test.csv"

        known_alloutput_path = base_path / "10.19_AllOutputs.csv"
        calcualted_alloutput_path = base_path / "10.19_AllOutputs_test.csv"
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for Emission Outputs - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known emission outputs file and calculated emission outputs file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
        ...
        self.assertEqual(calculated_linecount, known_linecount,
                         f"Known inputs file line count ({known_linecount}) does not match calculated inputs file line count ({calculated_linecount}) for All Outputs - SB4003 IDC 10.19")
        ...
        for n, row in enumerate(known):
            message = ('Row ' + str(
                n + 1) + ' of known all outputs file and calculated all outputs file are not equal - SB4003 IDC 10.19.')
            self.assertEqual(row, calculated[n], message)
    ...
    def test_energycalcs_3002_ISO(self):
        base_path = Path("./Unit Tests/super_pot")
    ...
    def test_energycalcs_3001_ISO(self):
        base_path = Path("./Unit Tests/BaselineTest_09062023")
